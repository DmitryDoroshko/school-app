...
define("COUNT_OF_STUDENT_FIELDS", 10);
...
if (isset($_GET['edit-student'])) {
    ...
    $update_student = true;
    $result = $mysqli->query("SELECT * FROM school WHERE student_id=$student_id") or die($mysqli->error);
...
if (isset($_GET['edit-language'])) {
    ...
    $result = $mysqli->query("SELECT * FROM foreign_language WHERE language_code=$language_id") or die($mysqli->error);
    $arr = $result->fetch_assoc();
    if (count($arr) == COUNT_OF_STUDENT_FIELDS) {
...
if (isset($_POST['update-language'])) {
    ...
    $mysqli->query("UPDATE foreign_language SET language_name='$foreign_language_name' 
                        WHERE language_code=$foreign_language_code") or die($mysqli->error);
